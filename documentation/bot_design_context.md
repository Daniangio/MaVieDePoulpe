...
Replay decision frames store the selected plan, planner settings, planner diagnostics, and the top five alternatives. AP and cards are discounted as the night reset approaches; generic late-night draws are suppressed, while draws required to finish an open interaction remain available. An open courtship cannot be failed while a known support-card draw or AP-collection path remains, so rollout scoring cannot replace that path with the fail/move/retry cycle. A shelter's third shell is preserved while courtship is pending.
